...
use vars qw(
   ...
   $TRANSMIT_HTML_IN_PARTS  @TRANSMIT_HTML_IN_PARTS_URLS
...
#   Thus, you also need to set @TRANSMIT_HTML_IN_PARTS_URLS below.
#$TRANSMIT_HTML_IN_PARTS= 1 ;

# When $TRANSMIT_HTML_IN_PARTS is set, then set this to a list of patterns
#   that describe the URLs you want to apply $TRANSMIT_HTML_IN_PARTS to.
#   The patterns work like @ALLOWED_SERVERS and @BANNED_SERVERS above, in that
#   they're lists of Perl 5 regular expressions.  See the comments there for
#   details.
...
# This is used when $TRANSMIT_HTML_IN_PARTS is in effect for the current URL.
#   It contains elements of http_get().
...
sub full_url {
	...
	return 'javascript:' . &proxify_js($script, 1) ;
    }
...
sub proxify_block {
	...
	# Proxify any strings inside "expression()" or "function()".
	# jsm-- this isn't rigorous-- breaks on ")" in expression.
	$s=~ s#((?:expression|function)\s*\()([^)]*)#
	       $1 . &proxify_js($2, 0)
	      #gie ;
...
sub full_insertion {
    ...
    $ret= "\n<div>\n$ret</div>\n\n<div id=\"_proxy_css_main_div\" style=\"position:absolute\">\n" ;
...
<td align=center>
<br />
<div class="Mcncc03"><b>Mcncc.com</b> v1.26 | 
动力达州 by <b>Powerdz.net</b>
<br />	
本代理IP:<font color=red>$ENV{'SERVER_ADDR'}</font> ' <u><font color="#FF9900">Mcncc</font></u> &copy; 2006-2007
</div>
</td>
<td align=right>
$rightlink
...
<div align="center">
	<div align="center">
		<table id="table13" borderColorDark="#666666" width="45%" bgColor="#ffffff" borderColorLight="#000000" border="1">
			<tr>
				<td >
				<div align="center">
					<table id="table14" style="BORDER-TOP-WIDTH: 0px; BORDER-BOTTOM-WIDTH: 0px; BORDER-RIGHT-WIDTH: 0px" width="97%" border="0">
						<tr>
							<td>
...
<input type=checkbox id="rc" name="rc"$rc_on><label for="rc"> 移除cookies</label>

</td>
</tr>
<tr>
	<td style="text-align: left">
<input type=checkbox id="rs" name="rs"$rs_on><label for="rs"> 移除scripts</label>

</td>
</tr>
<tr>
	<td style="text-align: left">
<input type=checkbox id="fa" name="fa"$fa_on><label for="fa">移除 ads</label>

</td>
</tr>
<tr>
	<td style="text-align: left">
<input type=checkbox id="br" name="br"$br_on><label for="br"> Hide referrer information</label>

</td>
</tr>
<tr>
	<td style="text-align: left">
<input type=checkbox id="if" name="if"$if_on><label for="if">显示地址</label>
...
<head>
<title>代理在线Mcncc.com——在线web代理——高速代理——proxy online——高匿代理——PHPROXY——在线web代理服务器</title>

<meta name="robots" content="index, nofollow" />
<style>
	body{font-family: Tahoma, arial; font-size: 14px; }
	input {font-family: tahoma, verdana; font-size: 11px; border: 1px solid #000000}
	select{font-family: tahoma, verdana; font-size: 11px; border: 1px solid #000000}
	a{color: #FFF;}
	a:hover{text-decoration: none}
	.Mcncc01 {font-size: 18pt; font-weight: bold; color:#FFF; text-align: center; margin-bottom: 5px; }
	.mcncc {font-family: tahoma; font-size: 12px; color:#FFF; }
	.Mcncc03 {font-family: verdana; font-size: 11px; color:#FFF; }
</style>

</head>

$msg
<body onload="document.getElementById('url').focus();"  bgcolor="#666666">
<body$onload>
<center>
<br>
<div class="Mcncc01">WEB在线代理-Powerdz.net 动力达州<br /><span class="mcncc">Mcncc.com</span></div>

<form name="URLform" action="$action" method=$method$onsubmit><span class="mcncc">输入地址:
<input name="URL" size=66 value="$safe_URL">
$flags
</span>

<p><script type="text/javascript"><!--
google_ad_client = "pub-0703022238563359";
google_ad_width = 468;
google_ad_height = 60;
...
</script><p><input type=submit value="   点此浏览   ">
</form>
EOF
...
$safe_URL2= "http://jmarshall.com/report.cgi?URL=$safe_URL2&version=2.1beta12" ;
...
sub proxify_js {
  ...
  OUTER:
	...
	#   the processing of a previous token.
		...
		if ($o ne '') {
		    push(@out, "_proxy_jslib_assign('$token', (" . &proxify_js($o, 0, $with_level) . "), (" . &proxify_js($p, 0, $with_level) . "), '')" ) ;
		} else {
...
#		$prefix= $token ;
	...
	#   return its argument unchanged if it's not a string.
	    ...
	    $term_so_far.= $token . $1 . '_proxy_jslib_proxify_js(('
			 . &proxify_js(&get_next_js_expr(\$in,1), 0, $with_level)
			 . "), 0, $with_level) )" ;
	    last unless $in=~ /\G\)/gc ;
	    $div_ok= 1 ;
	...
	#   "nodeValue" below will be handled regardless of object type.
		...
		$op= $1 ;
		$new_val= &proxify_js(&get_next_js_expr(\$in), 0, $with_level) ;
		if ($term_so_far eq '') {
		    push(@out, ($with_level
		...
		# Surround with parentheses if it starts a "new" expression.
		#   Otherwise, in some browsers, it won't create the new object
		#   correctly.  :P
		$term_so_far= "($term_so_far)" if $last_token eq 'new' ;
		$new_last_token= ')' ;
		$div_ok= 1 ;
	    }
	...
	#   this.
		...
		if ($next_expr=~ /^\s*\d+\s*$/) {
		    ...
		    $term_so_far.= $token . $skip1 . "$inc_by+(" . &proxify_js($next_expr, 0, $with_level) . ')]' ;
		} else {
		    $term_so_far.= $token . $skip1 . '(' . &proxify_js($next_expr, 0, $with_level) . ')]' ;
		}
		last unless $in=~ /\G\]/gc ;
		$new_last_token= ']' ;
	    } else {
	...
	} elsif ($token=~ /^(?:if|while|for|switch)$/) {
	    push(@out, $prefix, $term_so_far, $token) ;
	    $prefix= $term_so_far= '' ;
	    last unless $in=~ /\G($RE_JS_SKIP*\()/gco ;
	    push(@out, $1, &proxify_js(&get_next_js_expr(\$in,1), 0, $with_level), ')') ;
	    last unless $in=~ /\G\)/gc ;
	    $div_ok= 0 ;
	...
	#   declared and initialized if needed before the outermost with() statement.
	#   (If that happens to declare it globally, there is a possible race
	#   condition if the page uses multithreaded JavaScript.  Sorry.)
	...
	#   _proxy_jslib_with_handle() and _proxy_jslib_with_assign_rval().
	    ...
	    last unless $in=~ /\G($RE_JS_SKIP*)\(/gco ;
	    $skip1= $1 ;
	    $with_obj= &proxify_js(&get_next_js_expr(\$in, 1), 0, $with_level) ;
	    last unless $in=~ /\G\)($RE_JS_SKIP*)/gco ;
	    $skip2= $1 ;
	    if ($in=~ /\G\{/gc) {
		$code= '{' . &proxify_js(&get_next_js_expr(\$in, 1), 0, $with_level+1) . '}' ;
		last unless $in=~ /\G\}/gc ;
	    } else {
		# Note that a bare with() statement could still contain commas.
		$code= &proxify_js(&get_next_js_expr(\$in), 0, $with_level+1) ;
		$code.= ',' . &proxify_js(&get_next_js_expr(\$in), 0, $with_level+1)
		    while $in=~ /\G,/gc ;
	    }
	...
	#   after the identifier name.
		...
		push(@out, $skip1, $var) ;
		push(@out, $eq, &proxify_js($value, 0, $with_level)) if $eq ne '' ;
		last unless $in=~ /\G,/gc ;
		push(@out, ',') ;
	    }
	    $div_ok= 0 ;
	...
	#   for that case.
	# jsm-- must decrement _proxy_jslib_with_objs[] as necessary....
	    ...
	    $expr= &proxify_js($expr, 0, $with_level) ;
	    push(@out,
		 "return ((_proxy_jslib_ret= ($expr)), _proxy_jslib_flush_write_buffers(), _proxy_jslib_ret)") ;
	    $div_ok= 0 ;
	...
	# For "(", get inside parens, proxify, and add to output.
	    ...
	    $term_so_far.= '(' . &proxify_js(&get_next_js_expr(\$in,1), 0, $with_level) . ')' ;
	    last unless $in=~ /\G\)/gc ;
	    $new_last_token= ')' ;
	    $div_ok= 1 ;
	...
	#   if it looks like an array literal instead.
		...
		$sub_expr= &proxify_js(&get_next_js_expr(\$in,1), 0, $with_level) ;
		last unless $in=~ /\G\]/gc ;
		if ($term_so_far ne '') {
		    $needs_jslib= 1 ;
		    $new_last_token= ')' ;
...
function _proxy_jslib_init() {
	...
	window.onload= function() { _proxy_jslib_onload() } ;
    }
...
function _proxy_jslib_onload() {
    ...
    if (_proxy_jslib_is_in_frame && (window.parent===window.top) && top.proxy_jslib_insertion_frame)
...
//alert('starting handle; o, property, cur_val, URL=['+o+']['+property+']['+cur_val+']\n['+_proxy_jslib_URL+']') ;
    ...
    if (!o._proxy_jslib_is_proxified && _proxy_jslib_node_is_in_document(o))
...
//alert('starting cloneNode();')
		var ret= o.cloneNode(deep) ;
		ret._proxy_jslib_is_proxified= o._proxy_jslib_is_proxified ;
//alert('end of cloneNode() ;ret=['+_nodestring(ret)+']\n_p_j_is_proxified=['+ret._proxy_jslib_is_proxified+']') ;
		return ret ;
	      }
...
//alert('in Window.top; o, o.name, w, w.name=\n['+o+']['+o.name+']\n['+w+']['+w.name+']') ;  // jsm-- all alerts
	...
	case 'Document.getElementById':
	    ...
	    return function (elementId) {
		       ...
		       if (_proxy_jslib_write_buffers[i].doc!==o) return null ;
		       buf= _proxy_jslib_write_buffers[i] ;
	...
	case 'Document.getElementsByTagName':
	    return function (tagname) {
		       ...
		       for (i= 0 ; i<_proxy_jslib_write_buffers.length ; i++)
			   if (_proxy_jslib_write_buffers[i].doc===o) break ;
		       if (_proxy_jslib_write_buffers[i].doc!==o) return o.getElementsByTagName(tagname) ;
		       buf= _proxy_jslib_write_buffers[i] ;
...
if (o==null) alert('in assign, o is null, property, caller=\n['+property+']\n['+arguments.callee.caller+']') ;
	...
	case 'outerHTML':
	case 'outerText':
	    // avoid proxifying already-proxified HTML
	    if (opmod!='') {
		eval('new_val= o[property]' + opmod + 'val') ;
		eval('o[property]= o[property]' + opmod + '_proxy_jslib_proxify_html(val, false, (o.ownerDocument || o))[0]') ;
		return new_val ;
	    } else {
		o[property]= _proxy_jslib_proxify_html(val, false, (o.ownerDocument || o))[0] ;
		return val ;
	    }
...
function _proxy_jslib_proxify_html(html, is_full_page, doc) {
    ...
    if (html==void 0) return ['', false, false] ;
...
//sanity check
if (element._proxy_jslib_is_proxified) alert('called _proxy_jslib_proxify_element() on a proxified element; caller=['+arguments.callee.caller+']') ;
	...
	alert('Error: unhandled typeof==['+(typeof element)+ ']; caller=['+arguments.callee.caller+']') ;
	return ;
    }
...
function _proxy_jslib_proxify_css(css) {
	...
	out+= css.substr(0,m1.index) + m1[1] + _proxy_jslib_proxify_js(m1[3]) ;
	css= css.substr(m1.index+m1[0].length) ;
    }
    out+= css ;
...
function _proxy_jslib_proxify_js(s, top_level, with_level) {
  ...
  OUTER:
	...
	if (!token) {
	    ...
	    term_so_far+= token + match[1] + '_proxy_jslib_proxify_js(('
			+ _proxy_jslib_proxify_js(_proxy_jslib_get_next_js_expr(1), 0, with_level)
			+ '), 0, ' + with_level + ') )' ;
		...
		s= s.slice(match.index+match[0].length) ;
		op= match[3] ;
		new_val= _proxy_jslib_proxify_js(_proxy_jslib_get_next_js_expr(), 0, with_level) ;
		if (term_so_far=='') {
		    out[out.length]= (with_level
				      ? (token+"= _proxy_jslib_with_assign_rval(_proxy_jslib_with_objs, '', '"+token+"', '"+op+"', ("+new_val+"), "+token+")")
				      : (token+"= _proxy_jslib_assign_rval('', '"+token+"', '"+op+"', ("+new_val+"), "+token+")") ) ;
	...
	//} else if (/^(applets|embeds|forms|ids|layers|anchors|images|links)$/.test(token)) {
	    ...
	    s= s.slice(match.index+match[0].length) ;
	    out[out.length]= match[1] + _proxy_jslib_proxify_js(_proxy_jslib_get_next_js_expr(1), 0, with_level) + ')' ;
	    if (!(match= s.match(/^\)/))) break ;
	    s= s.slice(match.index+match[0].length) ;
	    div_ok= false ;
	    ...
	    s= s.slice(match.index+match[0].length) ;
	    skip1= match[1] ;
	    with_obj= _proxy_jslib_proxify_js(_proxy_jslib_get_next_js_expr(1), 0, with_level) ;
	    if (!(match= s.match('^\)(('+RE.SKIP+')*)'))) break ;
	    s= s.slice(match.index+match[0].length) ;
	    skip2= match[1] ;
	    if (match=s.match(/^\{/)) {
		s= s.slice(match.index+match[0].length) ;
		code= '{' + _proxy_jslib_proxify_js(_proxy_jslib_get_next_js_expr(1), 0, with_level+1) + '}' ;
		if (!(match= s.match(/^\}/))) break ;
		s= s.slice(match.index+match[0].length) ;
	    } else {
		code= _proxy_jslib_proxify_js(_proxy_jslib_get_next_js_expr(), 0, with_level+1) ;
		while (match=s.match(/^,/)) {
		    s= s.slice(match.index+match[0].length) ;
		    code+= ',' + _proxy_jslib_proxify_js(_proxy_jslib_get_next_js_expr(), 0, with_level+1) ;
		}
	    ...
	    does_write= 1 ;
	    term_so_far+= '(' + _proxy_jslib_proxify_js(_proxy_jslib_get_next_js_expr(1), 0, with_level) + ')' ;
	    if (!(match= s.match(/^\)/))) break ;
	    s= s.slice(match.index+match[0].length) ;
	    new_last_token= ')' ;
	    div_ok= true ;
	    ...
	    sub_expr= _proxy_jslib_proxify_js(_proxy_jslib_get_next_js_expr(1), 0, with_level) ;
	    if (!(match= s.match(/^\]/))) break ;
	    s= s.slice(match.index+match[0].length) ;
	    if (term_so_far) {
		_proxy_jslib_needs_jslib= true ;
...
function _proxy_jslib_object_type(o) {
	...
	if (o instanceof XMLHttpRequest) return 'XMLHttpRequest' ;
